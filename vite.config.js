...
export default defineConfig(({ command, isSsrBuild, mode }) => {
  const live = (loadEnv(mode, process.cwd(), 'VITE_').VITE_DATA_SOURCE || 'mock').toLowerCase() === 'api'
  return {
    plugins: [
      react(),
      {
        // Which data layer this build was made for, next to the manifest: scripts/budget.mjs measures the build that
        // is in dist/ against its own budget, whatever the shell or .env.local says when the check runs later.
        name: 'loom-build-info',
        apply: 'build',
        generateBundle() {
          if (!isSsrBuild) this.emitFile({ type: 'asset', fileName: '.vite/loom-build.json', source: JSON.stringify({ dataSource: live ? 'api' : 'mock' }) })
        },
      },
    ],
    /**
     ...
     */
    define: {
      __LOOM_API__: JSON.stringify(live),
