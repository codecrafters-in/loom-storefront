...
export default function DeliveryCheck({ variantId }) {
  ...
  return (
    <form onSubmit={check} className="mt-4 border-t border-line pt-4">
      <label htmlFor="delivery-postcode" className="mb-1.5 block text-[13px] font-medium">{t('Check delivery to your postcode')}</label>
      <div className="flex gap-2">
        {countries.length > 1 && (
          <select aria-label={t('Country')} className="field h-10 w-24 text-[13px]" value={country} onChange={(e) => setCountry(e.target.value)}>
        ...
        <p role="status" className="mt-2 flex items-start gap-2 text-[13px] leading-snug">
          ...
          <span>
            {state.result.deliverable
              ? date
                ? t('Delivers to {postcode} by {date} with {method}.', { postcode: state.asked, date, method: fastest.label })
                : fastest
                  ? t('Delivers to {postcode} with {method}.', { postcode: state.asked, method: fastest.label })
                  : t('Delivers to {postcode}.', { postcode: state.asked })
              : t('We don’t deliver to {postcode} yet.', { postcode: state.asked })}
          </span>
        </p>
      )}
